#v10: Easter.py wasn't defined
"""
ver_from_saved = read file
ver = read from Git

if ver != ver_from_saved: 
	
	download ver py files
...
for f in filelist:
	os.remove(icons_folder+f)		#only ver.py and readingfromfilelnx.py in here
...
if current_ver_num == git_ver_num: 
	...
	#verpyupdate = False #True, False test	#Ver14
	
	#remove existing ver_num file
	filelist = [ f for f in os.listdir(pi_folder) if (f.endswith('ver.txt'))]
	for f in filelist:
		os.remove(pi_folder+f)					#remove old ver.txt file
		
	#also remove other .py files in pi_folder and icons_folder except for ver.py (hold for now)	
	cmd = 'sudo rm ' + icons_folder + 'readingfromfilelnx_v' + str(current_ver_num) + '.py'
	os.system(cmd)
	
	cmd = 'sudo rm ' + pi_folder + 'Easter.py'		#MOD HERE v10
	os.system(cmd)
	
	cmd = 'sudo rm ' + pi_folder + 'Display_v' + str(current_ver_num) + '.py'
	os.system(cmd)
	
	#new ver 13 remove and update starup images
	startupfolder='/home/pi/startup/'
	filelist = [ f for f in os.listdir(startupfolder) if (f.endswith('.jpg'))]
	for f in filelist:
		os.remove(startupfolder+f)
			
	#update current_ver_num file
	...
	file2 = pi_folder + 'Easter.py'		#MOD HERE v10
	gitfile2 = 'Easter.py'				#MOD HERE v10
	...
	cmd = 'sudo python' + ' ' + icons_folder + 'readingfromfilelnx_v' + str(git_ver_num) + '.py' #need to include 
	os.system(cmd)
